...
def calculate_average_grade_sql(conn):
    #if fetchone bit not present, execute() returns a cursor object rather than the actual results. [0] as fetchone returns tuple (because sql is rows), so [0] gives us the actual value
    return conn.execute("SELECT AVG(grade) FROM tbl_student_grades").fetchone()[0] #we can use conn.execute directly rather than creating our own cursor explicitly as it's just a quick simple statement.
...
def calculate_average_attendance_sql(conn):
    return conn.execute("SELECT AVG(attendance) FROM tbl_student_grades").fetchone()[0] #again, fetchone used as above
...
def get_number_of_passes_sql(conn):
    return conn.execute("SELECT COUNT(*) FROM tbl_student_grades WHERE grade >= 40").fetchone()[0]
...
def get_number_of_As_sql(conn):
    return conn.execute("SELECT COUNT(*) FROM tbl_student_grades WHERE grade >= 70").fetchone()[0]
...
def get_number_of_Bs_sql(conn):
    return conn.execute("SELECT COUNT(*) FROM tbl_student_grades WHERE grade >= 60 and grade < 70").fetchone()[0] #inclusive left means lower bound is inclusive, upper bound is exclusive
...
def get_number_of_Cs_sql(conn):
    return conn.execute("SELECT COUNT(*) FROM tbl_student_grades WHERE grade >= 40 and grade < 60").fetchone()[0]
...
def get_average_grade_per_country_sql(conn):
    return conn.execute("SELECT COUNTRY, AVG(grade) FROM tbl_student_grades GROUP BY country").fetchall()
